...
const STORAGE_KEYS = {
  AGENTS: 'selfrefine-agents',
  MEMORY: 'selfrefine-memory',
  STATE: 'selfrefine-state'
};
...
const safeJSONParse = <T>(key: string, fallback: T): T => {
  try {
    ...
    // Convert date strings back to Date objects for memory entries
    if (key === STORAGE_KEYS.MEMORY && Array.isArray(parsed)) {
      return parsed.map((entry: MemoryEntry) => ({
        ...entry,
...
export function usePersistedWorkflow() {
  ...
  const [agents, setAgentsState] = useState<WorkflowAgent[]>(() => 
    safeJSONParse(STORAGE_KEYS.AGENTS, DEFAULT_AGENTS)
  );
  
  const [state, setState] = useState<AgentState>(() => 
    safeJSONParse(STORAGE_KEYS.STATE, initialState)
  );
  
  const [memory, setMemoryState] = useState<MemoryEntry[]>(() => 
    safeJSONParse(STORAGE_KEYS.MEMORY, [])
  );
  ...
  useEffect(() => {
    // Only persist completed states to avoid inconsistencies
    if (state.phase === 'complete' || state.phase === 'idle') {
      localStorage.setItem(STORAGE_KEYS.STATE, JSON.stringify(state));
    }
  }, [state]);

  const setAgents = useCallback((newAgents: WorkflowAgent[]) => {
    setAgentsState(newAgents);
  }, []);

  const setMemory = useCallback((newMemory: MemoryEntry[] | ((prev: MemoryEntry[]) => MemoryEntry[])) => {
